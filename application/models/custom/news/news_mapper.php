...
class News_mapper extends MY_Model implements Mapper {
    ...
    public function get_all_objects($parent_id = 0, $sort = 'desc', $sort_type = 'user_date') {
        $where      = $parent_id > 0 ? " where parent_id = {$parent_id}" : "";
        $sort       = $sort == 'asc' ? 'asc' : 'desc';
        $sql = "select id, parent_id, title, anno, spec_link, is_spec_link, novelty, action, description, image_id, inner_image, inner_position, unix_timestamp(date_created) created, unix_timestamp(user_date) user_date from {$this->_table_item} {$where} order by {$sort_type} {$sort}";
        $data = $this->db->query($sql)->result_array();
    ...
    public function get_object($id) {
        $sql = "select id, parent_id, title, anno, spec_link, is_spec_link, novelty, action, description, image_id, inner_image, inner_position, unix_timestamp(date_created) created, unix_timestamp(user_date) user_date from {$this->_table_item} where id = {$id}";
        $res = $this->db->query($sql)->row_array();
        if (sizeof($res) == 0) return false;
            ...
            if ($object->id > 0) {
                ...
                $sql = "update {$this->_table_item}
                        set title = {$this->db->escape($object->title)},
                            ...
                            novelty = {$this->db->escape($object->novelty)},
                            action = {$this->db->escape($object->action)},
    ...
    public function get_page_content($page_id = 0) {
        ...
        if ($page_id > 0 && $news_id == 0) {
            ...
            $sql            = "select n.id id, n.title title, n.anno anno, n.is_spec_link is_spec_link, n.novelty novelty, n.action action, n.spec_link spec_link, n.image_id image_id, n.inner_position inner_position, unix_timestamp(n.user_date) created, i.filename filename
            ...
            $sql                    = "select id, title, show_title, count_per_page from {$this->_table} where parent_id = {$page_id}";
            $news_category          = $this->db->query($sql)->row_array();
            $sql            = "select n.id, n.title, n.anno, n.description, n.image_id, n.novelty novelty, n.action action, n.inner_image, n.inner_position, unix_timestamp(n.user_date) created, i.filename filename from {$this->_table_item} n left join {$this->_table_image} i on n.image_id = i.id  where n.id = {$news_id}";
    ...
    public function get_rss($news_category_id = 0, $count = 100) {
        $sql = "select n.id id, n.title title, n.anno anno, n.is_spec_link is_spec_link, n.novelty novelty, n.action action, n.spec_link spec_link, unix_timestamp(n.user_date) created, p.url url
    ...
    public function get_widjet($count = 4) {
        $sql = "select n.id id, n.title title, n.anno anno, n.is_spec_link is_spec_link, n.novelty novelty, n.action action, n.spec_link spec_link, unix_timestamp(n.user_date) created, p.url url, i.filename filename
                from {$this->_table_item} n left join {$this->_table} nc on nc.id = n.parent_id left join {$this->_table_page} p on p.id = nc.parent_id
    ...
    protected function _get_object($data = array(), $type = 'item') {
        if ($type == 'category') {
            ...
            $tmp_object->novelty      = !empty($data['novelty'])                    ? $data['novelty']             : '';
            $tmp_object->action       = !empty($data['action'])                     ? $data['action']             : '';
